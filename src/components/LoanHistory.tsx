    ...
    return (
      <div className="bg-card border border-border rounded-xl p-6">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h3 className="font-semibold text-lg">{t('loan.historyTitle')}</h3>
        </div>
        <div className="text-center py-12">
          <History className="w-12 h-12 text-muted-foreground mx-auto mb-4" />
          <p className="text-muted-foreground">{t('loan.noLoanHistoryYet')}</p>
          <p className="text-sm text-muted-foreground mt-1">{t('loan.loansAppearHere')}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-card border border-border rounded-xl p-6">
      <div className="flex items-center gap-2 mb-6">
        ...
        <span className="ml-auto text-sm text-muted-foreground">
          {t('loan.totalCount').replace('{count}', String(allLoans.length))}
        </span>
      </div>
          ...
          return (
            <div key={loan.id} className="bg-muted/50 rounded-lg p-4 border border-border">
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="text-xs text-muted-foreground">{borrowDate.toLocaleDateString()}</span>
              </div>

              <div className="space-y-1 text-sm">
                <div className="flex justify-between text-muted-foreground">
                  <span>{t('loan.applicationDate')}</span>
                  <span>{borrowDate.toLocaleDateString()}</span>
                </div>
                {loan.status === 'repaid' && repaidDate && (
                  <div className="flex justify-between text-muted-foreground">
                    <span>{t('loan.repaidDate')}</span>
                    <span>{repaidDate.toLocaleDateString()}</span>
                  </div>
                )}
                {(loan.status === 'approved' || loan.status === 'overdue') && (
                  <div className="flex justify-between font-medium pt-2 border-t border-border">
                    <span>{t('loan.currentOwed')}</span>
                    <span>
                      {owed.total.toLocaleString(undefined, { minimumFractionDigits: 2 })} {loan.currency}
                    </span>
                  </div>
                )}
                {loan.status === 'repaid' && (
